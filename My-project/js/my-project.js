$(document).ready(function () {
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < 3; i++) {
                // console.log(i)
                var obj = new _carousel_list_movie('hot_movies', data);
            ...
            for (i = 0; i < 8; i++) {
                ...
                if (i <= 3) {
                    // console.log(i);
                    var obj = new _list_item_movie_desktop('top', 'tv_series', data);
                    obj.appendto_card_list();
                    var obj = new _list_item_movie_desktop('top', 'movies', data);
                    obj.appendto_card_list();
                } else {
                    // console.log(i);
                    var obj = new _list_item_movie_desktop('bottom', 'tv_series', data);
                    obj.appendto_card_list();
                    var obj = new _list_item_movie_desktop('bottom', 'movies', data);
                    obj.appendto_card_list();
                }
                var obj = new _carousel_list_movie('tv_series', data);
                obj.appendto_carousel_list();
                var obj = new _carousel_list_movie('movies', data);
                obj.appendto_carousel_list();
            }
        }
    });

    function _carousel_list_movie(carousel_type_movie, data) {

        this.carousel_type_movie = carousel_type_movie;
        this.data = data;
        if (this.carousel_type_movie === "hot_movies") {
            var new_data = data.hot_movies;
        } else if (this.carousel_type_movie === "tv_series") {
            var new_data = data.tv_series
        } else if (this.carousel_type_movie === "movies") {
    ...
    function _list_item_movie_desktop(pos_card_list_movie, type_movie, data) {
